...
namespace Microsoft.AspNetCore.Server.Kestrel.Core.Tests
{
    ...
    {
        [Fact]
        public void BlockedHeartbeatLoggedAsError()
        {
            ...
            {
                Assert.True(traceMre.Wait(TimeSpan.FromSeconds(1)));
                handlerMre.Set();
            }
        }

        [Fact]
        public void ExceptionFromHeartbeatHandlerLogged()
        {
            var systemClock = new MockSystemClock();
            var heartbeatHandler = new Mock<IHeartbeatHandler>();
            ...
            {
                // Heartbeat as a "dueTime" of zero, so a second should be more than enough time
                // for an error to be logged.
                Assert.True(kestrelTrace.Logger.MessageLoggedTask.Wait(TimeSpan.FromSeconds(1)));
            }
